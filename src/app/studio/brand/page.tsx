...
const LOCATIONS = ['The Villages (Main)', 'Summerfield Location', 'Ocala Showroom']

const CTAS = ['Call Us', 'Text Us', 'Visit Web']

export default function BrandPage() {
  const router = useRouter()
  const { session, userRole, setUploadedPhoto, setGeneration } = useStudio()
  ...
  return (
    <div className="h-full w-full flex flex-col p-6 sm:p-10">
        ...
        <div className="space-y-4">
          ...
          {userRole === 'employee' && (
            <div className="pt-4 border-t border-dashed border-stone-200">
              <label className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider mb-1.5 block">
      ...
      <div className="mt-auto pt-8">
        ...
        >
          {isLoading ? <Loader2 size={20} className="animate-spin" /> : 'Generate Preview Images'}
        </button>
      </div>

      <button onClick={() => router.push('/studio/style')} className="mt-4 text-sm text-stone-400 hover:text-stone-600 text-center">
        ← Back
      </button>
    </div>
  )
